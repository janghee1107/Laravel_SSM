		...
		<br>	
			...
			<table class="table  table-bordered  table-sm  table-hover mymargin5">
				<tr class="mycolor2">
					...
					@foreach ( $list  as  $row )   
<?
	$numi31 = $row->numi31 ? number_format($row->numi31) : "";
	$numo31 = $row->numo31 ? number_format($row->numo31) : "";
?>					

					<tr>
						<td>{{ $row->writeday31 }}</td>
						<td align="left">{{ $row->product_name31}}</td>
						<td align="right">{{ number_format($row->price31) }}</td>
						<td align="right" class="mycolor3">{{$numi31}}</td>
						<td align="right" class="mycolor3">{{$numo31}}</td>
						<td align="right">{{ number_format($row->prices31) }}</td>
						<td align="left">{{ $row->bigo31 }}</td>
					</tr>
					@endforeach
